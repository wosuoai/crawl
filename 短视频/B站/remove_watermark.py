...
@app.post("/deal_video_watermark/")
async def deal_video_watermark(remove_watermark_dto: RemoveWatermarkDto):
    remover = WatermarkRemover(roi_list=remove_watermark_dto.roi_list, threshold=remove_watermark_dto.threshold, kernel_size=remove_watermark_dto.kernel_size, video_path=remove_watermark_dto.video_path, save_path=remove_watermark_dto.save_path)
    remover.remove_video_watermark()
    return {"message": "Video watermark processed successfully."}


@app.post("/deal_video_subtitle/")
async def deal_video_subtitle(remove_watermark_dto: RemoveWatermarkDto):
    remover = WatermarkRemover(roi_list=remove_watermark_dto.roi_list, threshold=remove_watermark_dto.threshold, kernel_size=remove_watermark_dto.kernel_size, video_path=remove_watermark_dto.video_path, save_path=remove_watermark_dto.save_path)
...
if __name__ == "__main__":
    uvicorn.run(app="remove_watermark:app", host="0.0.0.0", port=8005, reload=True)