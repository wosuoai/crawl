...
class redbookImg:
    def __init__(self, host=redisHost, port=redisPort, password=redisPassword):
        ...
        self.service = ChromeService(executable_path=r'C:\Users\Administrator\.wdm\drivers\chromedriver\chromedriver.exe') # 配置谷歌操作驱动路径
    ...
    def get_target_link(self,keyword: str):
        ...
        yield user_id,item_num,target_link
    ...
    def page_items(self, link: str, num: int) -> list:
        ...
        for i in range((num//6)):
            ...
            yield item_link
            self.driver.switch_to.window(self.driver.window_handles[0]) #每次执行前回到主tab页，否则会找不到tab页
            # 网页滚动需要放到最后，渲染的数据里面只有当前页面的14个商品
            self.driver.execute_script(f'document.documentElement.scrollTop={(i + 2) * 1000}')
            time.sleep(1)

        return []

    # 解析每个商品里面的图片
    def get_item_html(self, items_url: str) -> str:
        img_url = [] #所有提取出来的图片链接存放列表
        requestUrl = items_url
        self.driver.switch_to.window(self.driver.window_handles[0]) #每次执行前回到主tab页，否则会找不到tab页
        # 执行JavaScript代码，在当前页面中打开一个新窗口
        self.driver.execute_script(f'window.open("{requestUrl}");')
        ...
        #     item_body = []
        # else:
        urls = re.findall(r'background\-image:\s*url\(([^\)]*)\)', item_body)
        for url in urls:
            if "http://sns-webpic-qc.xhscdn.com" in url:
            ...
            for imgUrl in imgList:
                if str(imgUrl).startswith("http"):
                    ...
                    try:
                        ...
                        img = Image.open(BytesIO(requests.get(imgUrl, headers=self.headers).content))
                        img.save("{}/{}.jpg".format(dirPath, filename))
                        print(dirPath + "图片{}下载成功".format(imgUrl))
                        #f.write(requests.get(imgUrl, headers=self.headers).content)
...
if __name__ == "__main__":
    ...
    searchItemImg.init_cookies_to_driver(cookies=cookies_list)  # 输入到cookies到driver
    ...
    for user_id,item_num,target_link in elements:
        ...
        for links in items_links:
            if links == []:
                ...
                for url in links:
                    # 判断是否需要进行去重
                    if searchItemImg.is_url_exist(url) == True:
                        print("该商品url已经爬取过,对url进行跳过")
                        continue

                    # 获取item页面的内容
                    result = searchItemImg.get_item_html(items_url=url)
                    for img_list,item_body,requestUrl in result:
                        ...
                        if img_list == []:
                            ...
                            for img in img_list:
                                ...
                                sql = "insert into all_imgs_copy4 (`shop_name`, `item_name`, `img_link`, `create_time`) values ('%s', '%s', '%s', '%s')" % (key_word + "--小红书", item_title, img, t)
                                cursor.execute(sql)
                                conn.commit()
