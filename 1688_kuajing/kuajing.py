...
class KJ:
    def __init__(self, host="127.0.0.1", port="3306", user="root", passwd="aifeibaihuo", database="goods") -> None:
        self.headers = {
        ...
        if fistCateInformation != {}:
            for fistCateItem in fistCateInformation["cate"]["result"]:
                ...
                self.exec_mysql("insert into 1688_kj_commodity_index \
                                 ...
                                 values ('%s','%s','%s','%s','%s','%s')" % (
                parentId, cateId, cateName, createdTime, 1, taskId))
                ...
                if secondCateInformation != {}:
                    for secondCateItem in secondCateInformation["cate"]["result"]:
                        ...
                        self.exec_mysql("insert into 1688_kj_commodity_index \
                                 (parent_id,cate_id,cate_name,created_time,cate_level,task_id)\
                                 values ('%s','%s','%s','%s','%s','%s')" % (
                        parentId, cateId, cateName, createdTime, 2, taskId))
                        ...
                        if threeCateInformation != {}:
                            for threeCateItem in threeCateInformation["cate"]["result"]:
                                ...
                                self.exec_mysql("insert into 1688_kj_commodity_index \
                                 (parent_id,cate_id,cate_name,created_time,cate_level,task_id)\
                                 values ('%s','%s','%s','%s','%s','%s')" % (
                                parentId, cateId, cateName, createdTime, 3, taskId))
                                ...
                                if fourCateInformation != {}:
                                    for fourCateItem in fourCateInformation["cate"]["result"]:
                                        ...
                                        (parent_id,cate_id,cate_name,created_time,cate_level,task_id)\
                                        values ('%s','%s','%s','%s','%s','%s')" % (
                                        parentId, cateId, cateName, createdTime, 4, taskId))
    ...
    def created_index_task(self, tasktype: int = 2) -> int:
        ...
        if tasktype == 1:
            ...
            # 创建任务
            taskId = self.exec_mysql_id(
                "insert into 1688_kj_commodity_task (created_time,type,task_describe) values ('%s','%s','%s')" % (
                createdTime, tasktype, describe))
            if taskId == 0:
    ...
    def save_commodity(self, item: dict):
        ...
        self.exec_mysql("insert into 1688_kj_commodity (`index_id`, `cate_level`, `cate_id`, `cate_name`, `gmt_create`, `offer_picurl`, `price`, `good_subject`, `offer_id`, `offer_url`, `created_time`, `total_page`, `task_id`, `rank`) \
                        ...
                        % (item["index_id"], item["cate_level"], item["cate_id"], item["cate_name"], item["gmt_create"],
                           item["offer_picurl"], item["price"], item["subject"], item["offer_id"], item["offer_url"],
                           item["created_time"], item["total_page"], item["task_id"], item["rank"]))

    # 定义一个获取商品信息表的方法
    def get_commodity(self, taskId):
        result = self.select_mysql(
            "SELECT commod.id,commod.offer_url,commod.rank FROM 1688_kj_commodity commod where id NOT IN (SELECT commodity_id FROM 1688_kj_commodity_detail ) and task_id='%s' order by commod.rank" % taskId)
        return result

    # 定义一个保存到商品详情表的方法
    def save_detail(self, item: dict):
        self.exec_mysql("insert into 1688_kj_commodity_detail (`commodity_id`,`business_name`,`user_evaluate`,`turnover`,`collect_num`,`created_time`,`task_id`, `rank`) \
                        values ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" \
                        % (item["commodity_id"], item["business_name"], item["user_evaluate"],
                           item["turnover"], item["collect_num"], item["created_time"], item["task_id"], item["rank"]))
    ...
    # 爬取详情页
    def spider_detail(self, taskId: int, cookies: list = [
        {'domain': '.1688.com', 'expiry': 1694873969, 'httpOnly': False, 'name': 'tfstk', 'path': '/',
         'sameSite': 'Lax', 'secure': False,
         'value': 'cefhBgbIi9JQUaNRfMOQiV1KQvJFZSweA_5N_PiAGfv2s15NiIMZ30n4SHEbYY1..'},
        {'domain': '.1688.com', 'expiry': 1679926773, 'httpOnly': False, 'name': 'is_identity', 'path': '/',
         'sameSite': 'Lax', 'secure': False, 'value': 'buyer'},
        {'domain': '.1688.com', 'expiry': 1679926773, 'httpOnly': False, 'name': 'aliwwLastRefresh', 'path': '/',
         'sameSite': 'Lax', 'secure': False, 'value': '1679321973471'},
        {'domain': '.1688.com', 'expiry': 1694873969, 'httpOnly': False, 'name': 'l', 'path': '/', 'sameSite': 'Lax',
         'secure': False,
         'value': 'fBE3ujjPNNMuDc6bKOfZPurza779SIRAguPzaNbMi9fP9H_B5yoCW1MToUA6CnGVFsi2R3o-P8F6BeYBqffjeFRtuQIXdpMmnmOk-Wf..'},
        {'domain': '.1688.com', 'httpOnly': False, 'name': '__mwb_logon_id__', 'path': '/', 'sameSite': 'Lax',
         'secure': False, 'value': 'tb80111606'},
        {'domain': '.1688.com', 'expiry': 1679393966, 'httpOnly': False, 'name': 'last_mid', 'path': '/',
         'sameSite': 'None', 'secure': True, 'value': 'b2b-345151005463a30'},
        {'domain': '.1688.com', 'httpOnly': False, 'name': '__cn_logon_id__', 'path': '/', 'sameSite': 'Lax',
         'secure': False, 'value': 'tb80111606'},
        {'domain': '.1688.com', 'expiry': 1679329177, 'httpOnly': False, 'name': '_show_user_unbind_div_', 'path': '/',
         'sameSite': 'Lax', 'secure': False, 'value': 'b2b-345151005463a30_false'},
        {'domain': '.1688.com', 'httpOnly': False, 'name': 'unb', 'path': '/', 'sameSite': 'None', 'secure': True,
         'value': '3451510054'},
        {'domain': '.1688.com', 'expiry': 1679926740, 'httpOnly': False, 'name': '_m_h5_tk', 'path': '/',
         'sameSite': 'None', 'secure': True, 'value': '05a223edc1c87ddb5042e2c17ecb30dc_1679332020508'},
        {'domain': '.1688.com', 'expiry': 1713881966, 'httpOnly': False, 'name': 'ali_apache_track', 'path': '/',
         'sameSite': 'Lax', 'secure': False, 'value': 'c_mid=b2b-345151005463a30|c_lid=tb80111606|c_ms=1'},
        {'domain': '.1688.com', 'httpOnly': True, 'name': 'csg', 'path': '/', 'sameSite': 'None', 'secure': True,
         'value': '71599057'},
        {'domain': '.1688.com', 'httpOnly': True, 'name': 'sgcookie', 'path': '/', 'sameSite': 'None', 'secure': True,
         'value': 'E100xUqn1kbQTwa92pq2qk8%2BkXlFv2mCjeEk2IGGn7T4ffkun16n6jnYr25eGpL0hxYJF3JxiJL%2FXqfeN8%2F%2Fak7LrEhaYPw4WyAopxX%2FjINUcrs%3D'},
        {'domain': '.1688.com', 'httpOnly': True, 'name': 'cookie17', 'path': '/', 'sameSite': 'None', 'secure': True,
         'value': 'UNQyQxMqUdx1nQ%3D%3D'},
        {'domain': '.1688.com', 'expiry': 1679329177, 'httpOnly': False, 'name': '_show_force_unbind_div_', 'path': '/',
         'sameSite': 'Lax', 'secure': False, 'value': 'b2b-345151005463a30_false'},
        {'domain': '.1688.com', 'httpOnly': True, 'name': 'cookie1', 'path': '/', 'sameSite': 'None', 'secure': True,
         'value': 'U%2BGWz3AsFiX%2BQb4KVw17j51DAUP9jxfiN9Dd%2FomAUJ8%3D'},
        {'domain': '.1688.com', 'httpOnly': True, 'name': 'uc4', 'path': '/', 'sameSite': 'None', 'secure': True,
         'value': 'nk4=0%40FY4GsvRHfRNKE%2BdeKAb%2BB7c9OCFj&id4=0%40UgP5GPE5h%2FvopPV87sbnzF8qm8ZD'},
        {'domain': '.1688.com', 'expiry': 1679926772, 'httpOnly': False, 'name': 'firstRefresh', 'path': '/',
         'sameSite': 'Lax', 'secure': False, 'value': '1679321972484'},
        {'domain': '.1688.com', 'httpOnly': False, 'name': 'mwb', 'path': '/', 'sameSite': 'Lax', 'secure': False,
         'value': 'ng'},
        {'domain': '.1688.com', 'expiry': 1713881983, 'httpOnly': False, 'name': 'alicnweb', 'path': '/',
         'sameSite': 'Lax', 'secure': False, 'value': 'touch_tb_at%3D1679321950680%7Clastlogonid%3Dtb80111606'},
        {'domain': '.1688.com', 'httpOnly': False, 'name': '_csrf_token', 'path': '/', 'sameSite': 'None',
         'secure': True, 'value': '1679321966667'},
        {'domain': '.1688.com', 'expiry': 1679329178, 'httpOnly': False, 'name': '__rn_alert__', 'path': '/',
         'sameSite': 'Lax', 'secure': False, 'value': 'false'},
        {'domain': '.1688.com', 'expiry': 1679329177, 'httpOnly': False, 'name': '_show_sys_unbind_div_', 'path': '/',
         'sameSite': 'Lax', 'secure': False, 'value': 'b2b-345151005463a30_false'},
        {'domain': '.1688.com', 'expiry': 1679329174, 'httpOnly': False, 'name': '_is_show_loginId_change_block_',
         'path': '/', 'sameSite': 'Lax', 'secure': False, 'value': 'b2b-345151005463a30_false'},
        {'domain': '.1688.com', 'expiry': 1713881943, 'httpOnly': False, 'name': 'cna', 'path': '/', 'sameSite': 'None',
         'secure': True, 'value': 'VlmfHHEGT04CASe8CIWdK7cd'},
        {'domain': '.1688.com', 'httpOnly': False, 'name': '_tb_token_', 'path': '/', 'sameSite': 'None',
         'secure': True, 'value': 'ffb7596b3816e'},
        {'domain': '.1688.com', 'httpOnly': False, 'name': '__cn_logon__', 'path': '/', 'sameSite': 'Lax',
         'secure': False, 'value': 'true'},
        {'domain': '.1688.com', 'expiry': 1710857966, 'httpOnly': False, 'name': 'lid', 'path': '/', 'sameSite': 'None',
         'secure': True, 'value': 'tb80111606'},
        {'domain': '.1688.com', 'httpOnly': False, 'name': 'sg', 'path': '/', 'sameSite': 'None', 'secure': True,
         'value': '641'},
        {'domain': '.1688.com', 'expiry': 1679408342, 'httpOnly': False, 'name': 'xlly_s', 'path': '/',
         'sameSite': 'None', 'secure': True, 'value': '1'},
        {'domain': '.1688.com', 'expiry': 1694873972, 'httpOnly': False, 'name': 'isg', 'path': '/', 'sameSite': 'None',
         'secure': True, 'value': 'BIqKdG5R2t1bFVZxL2XWLeoN23Asew7VWe7BXxTDNl1oxyqB_Ate5dC10zMbN4Zt'},
        {'domain': '.1688.com', 'httpOnly': False, 'name': 'ali_apache_tracktmp', 'path': '/', 'sameSite': 'Lax',
         'secure': False, 'value': 'c_w_signed=Y'},
        {'domain': '.1688.com', 'expiry': 1679926740, 'httpOnly': False, 'name': '_m_h5_tk_enc', 'path': '/',
         'sameSite': 'None', 'secure': True, 'value': '3bf670c1e6240d9b6784bc494baff747'},
        {'domain': '.1688.com', 'httpOnly': False, 'name': 't', 'path': '/', 'sameSite': 'None', 'secure': True,
         'value': '5030ddc26b152ff4b5b7798614f9f3ff'},
        {'domain': '.1688.com', 'expiry': 1679926772, 'httpOnly': False, 'name': 'lastRefresh', 'path': '/',
         'sameSite': 'Lax', 'secure': False, 'value': '1679321972484'},
        {'domain': '.1688.com', 'httpOnly': False, 'name': '_nk_', 'path': '/', 'sameSite': 'None', 'secure': True,
         'value': 'tb80111606'},
        {'domain': '.1688.com', 'httpOnly': True, 'name': 'cookie2', 'path': '/', 'sameSite': 'None', 'secure': True,
         'value': '1bb8189afb73e70d741d44ae714785e3'}]):
    ...
    def parse_detail(self, commodId: int, commodUrl: str, cookies: list, rank: int, taskId: int) -> None:
        ...
        if obj.scrape_page(condition=EC.visibility_of_element_located, locator=(
        By.XPATH, '//*[@id="10811813010580"]/div/div[2]/div[1]/div/div[1]/div[1]/div/div/div/ul/li[2]/div')):
            ...
            try:
                ...
                # 收藏数
                collectNum = obj.scrape_item_by_path('//div[@class="no-affix-wrapper"]/div[2]/div[2]/div/div/div[4]/span').text
                collectNum = re.sub("\D", "", collectNum)
                commodId = commodId
                createdTime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
